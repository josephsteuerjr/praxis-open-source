...
class BroadPoolTests(unittest.TestCase):
    ...
    def test_broken_disk_pool_is_ignored(self):
        """БИТАЯ запись игнорируется. Просто старая — уже нет.

        02.08.2026, решение Праксис: «последнее готовое семя не выбрасывать только из-за
        возраста… пустоту отдавать только если валидного seed ещё никогда не
        существовало». Прежняя посылка «просрочено = негодно» отменена ею сознательно:
        именно она делала кэш непопадающим (семя жило час, а между её явными «вспомни»
        проходят часы), и слой стоял выключенным месяцами.

        Все три полезные нагрузки ниже — именно БИТЫЕ: метка из 1970 года, не-json и
        чужой ключ скоупа с меткой из будущего. Ни одна из них не описывает реальный
        пересчёт, поэтому проверка остаётся в силе целиком.
        """
